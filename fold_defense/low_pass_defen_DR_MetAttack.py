...
if opt.dataset == 'cora':
    attack_info = t.load("../fold_attack/GCN/MetAttack/attack_info_10".format(opt.dataset))
else:
    attack_info = t.load("../fold_attack/GCN/MetAttack/attack_info_{}_10".format(opt.dataset))


adj_per = attack_info['adj_per'].A
...
hidden = adj_per_nor_t @ hidden @ model.gc1.weight.cuda()
if model.with_bias:
    hidden = hidden + model.gc1.bias
if model.with_relu:
    hidden = F.relu(hidden)
hidden = adj_per_nor_t @ hidden @ model.gc2.weight.cuda()
if model.with_bias:
